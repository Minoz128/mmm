...
<article class="page-container">
    <form class="form form-horizontal" id="form-mmm-add">
        <?php if(is_array($tablefields) || $tablefields instanceof \think\Collection || $tablefields instanceof \think\Paginator): $i = 0;$__LIST__ = is_array($tablefields) ? array_slice($tablefields,1,null, true) : $tablefields->slice(1,null, true); if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <div class="row cl">
                <label class="form-label col-xs-6 col-sm-5"><span class="c-red">*</span><?php echo $vo['Comment']; ?>（<?php echo $vo['Type']; ?>）：</label>
                <div class="formControls col-xs-6 col-sm-7">
                    <input type="text" class="input-text" value="" placeholder="填写<?php echo $vo['Comment']; ?>" id="<?php echo $vo['Field']; ?>" name="<?php echo $vo['Field']; ?>">
                </div>
            </div>
...
<script>
    var SCOPE = {
        'form_add_url' : "<?php echo url('parameter/addexperiencegroups'); ?>",
    }
</script>
</html>